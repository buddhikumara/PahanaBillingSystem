...
<div class="container py-4">
  <div class="hero p-4 mb-4 d-flex flex-wrap justify-content-between align-items-center gap-3">
    ...
    <div class="d-flex gap-2">
      ...
      <!-- Add allowed: ADMIN, USER (not CASHIER) -->
      <c:if test="${authUser.role == 'ADMIN' || authUser.role == 'USER'}">
        <button class="btn btn-dark btn-rounded" data-bs-toggle="modal" data-bs-target="#addModal">+ Add Item</button>
      </c:if>
    </div>
  </div>
        ...
        <form class="needs-validation" novalidate method="post" action="${pageContext.request.contextPath}/items">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input id="edit-cost" name="costPrice" type="number" min="1" max="10000" step="0.01"
                     class="form-control" required value="${form.costPrice}">
            </div>
            <div class="mb-3">
              <label class="form-label">Retail price</label>
              <input id="edit-retail" name="retailPrice" type="number" min="1" max="10000" step="0.01"
                     class="form-control" required value="${form.retailPrice}">
            </div>
            <div class="mb-3">
              <label class="form-label">Quantity</label>
              <input id="edit-qty" name="quantity" type="number" min="0" step="1"
                     class="form-control" value="${form.quantity}">
            </div>
          </div>
      ...
      <form class="needs-validation" novalidate method="post" action="${pageContext.request.contextPath}/items">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            <label class="form-label">Cost price</label>
            <!-- Cost price -->
            <input name="costPrice" type="number" min="1" max="10000" step="0.01"
                   class="form-control" required value="${form.costPrice}"
                   title="Enter 1.00–10000.00, up to 2 decimals">
            <div class="invalid-feedback">Cost price is required (1.00–10000.00, up to 2 decimals).</div>

            <!-- Retail price -->
            <input name="retailPrice" type="number" min="1" max="10000" step="0.01"
                   class="form-control" required value="${form.retailPrice}"
                   title="Enter 1.00–10000.00, up to 2 decimals">
            <div class="invalid-feedback">Retail price is required (1.00–10000.00, up to 2 decimals).</div>
          <div class="mb-3">
            <label class="form-label">Retail price</label>
            <input id="edit-retail" name="retailPrice" type="number" min="1" max="10000" step="0.01"
                   class="form-control" required value="${form.retailPrice}"
                   title="Enter 1.00–10000.00, up to 2 decimals">
            <div class="invalid-feedback">Retail price must be 1.00–10000.00 (max 2 decimals).</div>
          <div class="mb-3">
            <label class="form-label">Quantity</label>
            <input id="edit-qty" name="quantity" type="number" min="0" step="1" class="form-control" value="${form.quantity}">
            <div class="invalid-feedback">Quantity must be a number ≥ 0.</div>
          </div>
        </div>
...
<script>
  ...
  // If server asked to reopen modal after validation errors
  (() => {
