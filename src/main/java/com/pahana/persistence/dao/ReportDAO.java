...
public class ReportDAO {
    ...
    private static Timestamp startOfDay(LocalDate d) {
        return Timestamp.valueOf(d.atStartOfDay());
    }
    private static Timestamp nextDay(LocalDate d) {
        // [from, nextDay) window
        return Timestamp.valueOf(d.plusDays(1).atStartOfDay());
    }
    private static Timestamp rangeStart(LocalDate from) {
        return startOfDay(from);
    }
    private static Timestamp rangeEndExclusive(LocalDate to) {
        return nextDay(to);
    }
    ...
    public Map<String,Object> summary(Connection c, LocalDate from, LocalDate to) throws SQLException {
        ...
        String payCol = findColumn(c, "bills", "payment_method");
        String discCol = findColumn(c, "bills", "discount");
        String paidCol = findColumn(c, "bills", "paid_amount");

        // bills_count, gross, discount_total?, net
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT COUNT(*) bills_count, ")
                .append("COALESCE(SUM(total_amount),0) gross");
        ...
        try (PreparedStatement ps = c.prepareStatement(sb.toString())) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    double disc = discCol == null ? 0.0 :
                            (rs.getBigDecimal("discount_total") == null ? 0.0 : rs.getBigDecimal("discount_total").doubleValue());
                    out.put("gross", gross);
        ...
        // items_sold
        String sqlItems = "SELECT COALESCE(SUM(bi.quantity),0) items_sold " +
        ...
        if (payCol != null) {
            String s = "SELECT " +
                    "COALESCE(SUM(CASE WHEN " + payCol + "='CASH' THEN total_amount END),0) cash_total, " +
                    "COALESCE(SUM(CASE WHEN " + payCol + "='CARD' THEN total_amount END),0) card_total " +
        ...
        // top item
        String topSql = "SELECT i.item_id, i.item_name, SUM(bi.quantity) qty, COALESCE(SUM(bi.total_price),0) amount " +
                "FROM bill_items bi JOIN items i ON i.item_id=bi.item_id " +
